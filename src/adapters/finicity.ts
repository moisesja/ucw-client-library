...
export function mapTransaction(t: { type: any; amount: any; id: any; postedDate: any; transactionDate: string | number | Date; description: any; memo: any; Category: any; Status: any; categorization: { normalizedPayeeName: any; }; checkNum: any; }, accountId: string){
    // Deposit Transaction
...
export function mapAccount(a: { type: any; id: any; number: any; realAccountNumberLast4: any; status: any; currency: any; name: any; balance: any; detail: { availableBalanceAmount: any; }; balanceDate: string | number | Date; }){
...
export function mapIdentity(customerId: string, accountOwnerDetail: { addresses: any; ownerName: string; OwnerName: any; emails: { email: string; }[]; phone: { type: any; country: any; phone: any; }[]; }){
    let addresses = [];
    ...
    if(parsedName.error?.length && parsedName.error?.length > 0){
    ...
    let ret = {
      ...
      addresses,
      email: accountOwnerDetail.emails?.map((e: { email: string; }) => e.email),
      telephones: accountOwnerDetail.phone?.map((p: { type: any; country: any; phone: any; }) => ({
        type: p.type,
        country: p.country,
        number: p.phone
      })),
 ...
 export function mapFiAttributes(fi: { name: string; }){
    return [
      {
        name: 'name',
        value: fi.name
