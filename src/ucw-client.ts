...
import { getProviderConfiguration } from "./provider-configuration";
...
export class UCWClient {
  ...
  constructor(config: Config) {
    ...
    const providerConfig = getProviderConfiguration(config);

    this.akoyaClient = new AkoyaClient(providerConfig.akoyaProd, config);
    this.finicityClient = new FinicityClient(
      providerConfig.finicityProd,
      config
    );
    this.mxClientInt = new MxVcClient(providerConfig.mxInt);
    this.mxClientProd = new MxVcClient(providerConfig.mxProd);
    this.sophtronClient = new SophtronVcClient(providerConfig.sophtron);
  }
  ...
  async post(path: string, data: any) {
    ...
    const res = await axios.post(this.config.AuthServiceEndpoint + path, data, {
  ...
  async getAuthCode() {
    ...
    const ProviderCredentials = getProviderConfiguration(this.config);
