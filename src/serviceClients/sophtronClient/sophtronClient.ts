...
export class SophtronClient extends SophtronBaseClient {
    ...
    if ((name || "").length > 0) {
      ...
      if (data?.length > 0) {
        return data
          .sort(
            (a: any, b: any) =>
              a.InstitutionName.length - b.InstitutionName.length
          )
          .slice(0, 9);
      }
      return data;
  ...
  createUserInstitutionWithProfileInfo(
    username: string,
    password: string,
    institutionId: string
  ) {
  ...
  createUserInstitutionWithAllPlusProfile(
    username: string,
    password: string,
    institutionId: string
  ) {
  ...
  createUserInstitutionWithFullHistory(
    username: string,
    password: string,
    institutionId: string
  ) {
  ...
  getUserInstitutionProfileInfor(userInstitutionID: string) {
    ...
    return this.post(url, { UserInstitutionID: userInstitutionID }).then(
      (data) => {
  ...
  ping = () => {
    return http.get(`${this.apiConfig.endpoint}/UserInstitution/Ping`);
  };
}
